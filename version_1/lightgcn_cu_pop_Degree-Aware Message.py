...
# + Method A: Degree-Aware Message Normalization integrated inside message passing.
# Nothing else added (no fairness term, no extra losses).
...
class CFG:
    ...
    out_dir: str = str((SCRIPT_DIR / "dataset" / "lightgcn_pipeline_cu_message_parent").resolve())
    ...
    # Supports:
    #   - user_id,credibility
    #   - user_idx,credibility  (ONLY if user_idx matches LightGCN user2idx!)
...
def load_credibility_vector(cred_csv_path: str, user2idx: dict) -> np.ndarray:
    """
    Returns cred[num_users] float32 in [0,1].
    Missing users -> 1.0 (no change).
    ...
    print(f"[CRED] stats: min={cred.min():.4f} p10={p10:.4f} p50={p50:.4f} p90={p90:.4f} p99={p99:.4f} max={cred.max():.4f}")
    return cred
...
# =========================
# Build credibility-weighted propagation matrices
# + Method A: Degree-aware item damping alpha_i = 1/log(1+deg(i))
# =========================
def build_message_passing_mats(train_edges_2xE: np.ndarray, num_users: int, num_items: int, cred_u: torch.Tensor, device: str):
    """
    Builds two sparse mats:
      M_ui: [U, I]  user <- item  (standard normalized, but with popularity damping alpha_i)
      M_iu: [I, U]  item <- user  (credibility-weighted on users, and popularity damping alpha_i)

    Normalization uses 1/sqrt(deg_u * deg_i).
    For Method A:
      alpha_i = 1/log(1 + deg_i)
      applied on both directions via multiplying edge weights by alpha_i.
    For M_iu:
      additionally multiply by c_u per edge.
    """
    u = train_edges_2xE[0].astype(np.int64)  # [E]
    i = train_edges_2xE[1].astype(np.int64)  # [E]
    ...
    # base norm weights per edge: 1/sqrt(deg_u * deg_i)
    w_base = inv_sqrt_u[u] * inv_sqrt_i[i]  # [E]

    # -------------------------
    # Method A (popularity damping):
    # alpha_i = 1/log(1 + deg_i)
    # safe: deg_i>=0; log1p(0)=0 -> avoid div by 0 by max with eps
    # -------------------------
    alpha_i = 1.0 / np.log1p(np.maximum(deg_i, 1.0))  # [I]
    alpha_i = alpha_i.astype(np.float32)

    # apply alpha_i per edge
    w_pop = w_base * alpha_i[i]  # [E]

    # M_ui: user receives from items (pop-damped)
    idx_ui = torch.tensor(np.vstack([u, i]), dtype=torch.long, device=device)
    val_ui = torch.tensor(w_pop, dtype=torch.float32, device=device)
    M_ui = torch.sparse_coo_tensor(idx_ui, val_ui, size=(num_users, num_items)).coalesce()

    # M_iu: item receives from users (credibility + pop-damped)
    c_u_np = cred_u.detach().view(-1).cpu().numpy().astype(np.float32)
    w_cred_pop = c_u_np[u] * w_pop  # [E]

    idx_iu = torch.tensor(np.vstack([i, u]), dtype=torch.long, device=device)
    val_iu = torch.tensor(w_cred_pop, dtype=torch.float32, device=device)
    M_iu = torch.sparse_coo_tensor(idx_iu, val_iu, size=(num_items, num_users)).coalesce()

    # diagnostics
    nz_i = int(np.count_nonzero(deg_i))
    print(f"[POP] item_deg: min={deg_i.min():.0f} max={deg_i.max():.0f} mean={deg_i.mean():.2f} nonzero={nz_i}/{num_items}")
    print(f"[POP] alpha_i:  min={alpha_i.min():.6f} median={np.median(alpha_i):.6f} max={alpha_i.max():.6f}")

    return M_ui, M_iu


# =========================
# LightGCN with alternating bipartite propagation (unchanged)
# =========================
class LightGCN(torch.nn.Module):
    ...
    def propagate(self):
        ...
        for _ in range(self.num_layers):
            i = torch.sparse.mm(self.M_iu, u)   # item <- user (cred + pop)
            u = torch.sparse.mm(self.M_ui, i)   # user <- item (pop)
            u_list.append(u)
            i_list.append(i)
